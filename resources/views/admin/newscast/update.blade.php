...
<form action="{{route('xu-li-cap-nhat-ban-tin',['id'=>$newsCast->id])}}" method="post" enctype="multipart/form-data" id="update-newscast">
    ...
    <div class="form-group">
        ...
        <select class="form-select" aria-label="select account" name="account_id">
            ...
            @if ($account->id==$newsCast->account_id)
    ...
    <div class="form-group">
        <label for="type_id">Loại bản tin</label>
        <select class="form-select" aria-label="select account" name="type_id">
            @foreach ($listTypeNewsCast as $typeNewsCast)
            @if ($newsCast->type_id==$typeNewsCast->id)
            <option value="{{$typeNewsCast->id}}" selected>{{$typeNewsCast->name}}</option>
            @else
